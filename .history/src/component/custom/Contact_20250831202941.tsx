...
const Contact = () =>{
    ...
    const ContactListComponents = useMemo(async ()=>{
        return ContactList.map((el)=>{
            return <div key={"contact-"+Math.random()}>{el.name}: <a className="underline hover:text-white" href={el.link}>{el.alias}</a></div>
        })
    },[])   
